...
object CalcDriverAge {

  case class MpdAutoDriverCDA(riskStateCd: Long, ratingPlan: String, plcyPeriodId: Long, driverBirthDt: Long, driverLicenseDt: Long, driverAge: Int)
  ...
  def main(args: Array[String]) {
    val LOG = LoggerFactory.getLogger("CalcDriverAge")
    val inDataMAD = Seq(
    ...
    val inDrLicDtLEPlcyDt = col("driverLicenseDt") <= col("plcy_efdt")
    val dtChk = udf(new EppUtils().isValidDate _)
    val validVersionDt = dtChk(col("versionDt"), lit("yyyyMMdd"))
    val validDriverBirthDt = dtChk(col("driverBirthDt"), lit("yyyyMMdd"))
    val validPlcyEfDt = dtChk(col("plcy_efdt"), lit("yyyyMMdd"))
    val dtFmt = udf(new EppUtils().formatDate _)
    val diffVersionBirthDt = datediff(dtFmt(col("versionDt"), lit("yyyyMMdd")), dtFmt(col("driverBirthDt"), lit("yyyyMMdd")))
    val diffPlcyEfBirthDt = datediff(dtFmt(col("plcy_efdt"), lit("yyyyMMdd")), dtFmt(col("driverBirthDt"), lit("yyyyMMdd")))
    ...
    val outDF = inDFMAD.join(inDFMAPP, inDFMAD.col("riskStateCd") === inDFMAPP.col("riskStateCd")
      ...
      .withColumn("UpdDriverAge", when(inDrLicDtGTPlcyDt, when(validVersionDt and validDriverBirthDt, diffVersionBirthDt).otherwise(lit("0")))
                                 .when(validPlcyEfDt and validDriverBirthDt, diffPlcyEfBirthDt).otherwise(lit("0")))
    outDF.show()
